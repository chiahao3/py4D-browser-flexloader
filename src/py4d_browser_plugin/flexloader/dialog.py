...
class FlexLoaderDialog(QDialog):
    ...
    def select_file(cls, parent) -> str | None:
        filename, _ = QFileDialog.getOpenFileName(
            parent,
            "Open HDF5 or Zarr Data",
            "",
            "HDF5/Zarr (*.h5 *.hdf5 *.emd *.mat *.py4dstem *.zarr *.zip);;Any file (*)",
        )
        if filename:
            return filename
    ...
    def _build_mapping_controls(self) -> None:
        ...
        headers = ["Axis", "Size", "Role", "Fixed Index"]
        ...
        for axis, size in enumerate(node.shape):
            ...
            role_box.currentTextChanged.connect(self._update_fixed_box_states)
            self.role_boxes.append(role_box)

            index_box = QSpinBox()
            index_box.setRange(0, max(0, int(size) - 1))
            index_box.setEnabled(defaults[axis] == "Fixed Index")
    ...
    def _update_fixed_box_states(self) -> None:
        for role_box, index_box in zip(self.role_boxes, self.index_boxes):
            index_box.setEnabled(role_box.currentText() == "Fixed Index")
    ...
    def _update_load_enabled(self) -> None:
        ...
        if node is None or not node.is_array or node.shape is None:
            self.load_button.setEnabled(False)
            return
        roles = [box.currentText() for box in self.role_boxes]
        assigned = [role for role in roles if role != "Fixed Index"]
        ok = all(role in assigned for role in DATACUBE_ROLES) and len(set(assigned)) == 4
